...
def mix_audio(voice_bytes: bytes, music_bytes: bytes) -> bytes:
    ...
    fade_duration = min(2000, len(music) // 4)
...
def generate_audio(data: AudioRequest, current_user: dict = Depends(get_current_user)):
    ...
    if not is_admin:
        ...
        if data.tom not in rules["voices"]:
            raise HTTPException(status_code=403, detail=f"Voz '{data.tom}' não disponível no seu plano.")
        if len(data.texto) > rules["max_chars"]:
            raise HTTPException(status_code=403, detail=f"Texto excede o limite de {rules['max_chars']} caracteres.")
...
async def generate_audio_with_music(
    ...
    if not is_admin:
        if current_user["credits"] < 3:
            raise HTTPException(status_code=402, detail="Créditos insuficientes. A mixagem com música custa 3 créditos.")
        rules = PLAN_RULES.get(plan, PLAN_RULES["free"])
        if tom not in rules["voices"]:
            raise HTTPException(status_code=403, detail=f"Voz '{tom}' não disponível no seu plano.")
    ...
    return {"audio_url": audio_url, "message": "Áudio com música gerado com sucesso"} 
